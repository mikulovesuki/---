...
图 \ref{fig:sensitivity_analysis} 展示了关键参数扰动对年度净减排量 $E_{net}$ 的影响。该图通过不同情景的柱状图与折线图对比，直观呈现了避免排放因子、废弃物转移率、运输参数和消费者交通排放因子四类敏感性测试结果。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth]{7f685b22205e0bfb2945010b0086c9c1.png}
\caption{关键参数敏感性分析结果示意图}
\label{fig:sensitivity_analysis}
\end{figure}

\subsection{消费奖励机制}%任务三

\subsubsection{核心规则：基于避免排放的1:1碳积分}

本模型量化了折扣零售通过废弃物转移率 $R=0.90$ 实现的避免排放$E_{\text{avoid}}=148,\!500\ \mathrm{kg\,CO_2e/年}$。积分规则直接以该避免排放量为锚：

\begin{itemize}
    \item \textbf{基础兑换率}：每避免 $1\ \mathrm{kg\,CO_2e}$ 排放，消费者获得 $1$ 个碳积分。
            该兑换率确保积分的发放与实际减排贡献实现等价映射，增强消费者对环保行为的认同感。
    \item \textbf{分类系数}：
         食品类系数 $1.0$（基准）；非食品类系数 $1.2$（上浮）。
         其合理性在于非食品（塑料、洗护用品等）的废弃处置碳足迹更高，给予额外激励可引导消费结构优化。
    \item \textbf{阶梯奖励}（月累计基础积分）：
         $\geq 50$ 积分——额外奖励 $+10\%$；
         $\geq 120$ 积分——额外奖励 $+20\%$。
         阶梯设计鼓励高频消费，提升月均购买次数，从而放大总避免排放量。
    \item \textbf{月积分封顶}：$500$ 积分/月，防止积分超发，保持财务可持续。
\end{itemize}

\subsubsection{积分计算示例}

典型消费者月购分析（与任务一、二模型参数一致）：月购食品 $5\ \mathrm{kg}$、非食品 $1\ \mathrm{kg}$，$R=0.90$。

\begin{itemize}
    \item 避免排放：
    \[
    \text{食品}：5\times2.7\times0.9=12.15\ \mathrm{kg\,CO_2e},\quad
    \text{非食品}：1\times3.0\times0.9=2.70\ \mathrm{kg\,CO_2e}
    \]
    \item 基础积分：食品 $12.15$ 分，非食品 $2.70\times1.2=3.24$ 分，合计 $15.39$ 分。
    \item 未触发阶梯（$<50$），月总积分 $=15.39$。
\end{itemize}

若用户升级为高频消费（月购扩大3倍），月基础积分约 $46$，触发阶梯奖励后 $\approx51$ 分，可兑换 $5$ 元折扣券。表 \ref{tab:consumer_credit_example} 给出了更详细的月度积分核算。

\begin{table}[htbp]
\centering
\caption{典型消费者月度积分核算示例}
\label{tab:consumer_credit_example}
\begin{tabular}{|l|c|c|c|c|}
\hline
消费类别 & 购买量(kg) & 排放因子 & 避免排放量(kg\,CO$_2$e) & 基础积分 \\
\hline
食品（零食/饮料） & 2.5 & 2.7 & 6.75 & 6.75 \\
\hline
食品（乳制品）   & 1.5 & 2.7 & 4.05 & 4.05 \\
\hline
食品（烘焙）     & 1.0 & 2.7 & 2.70 & 2.70 \\
\hline
非食品（洗护用品）& 1.0 & 3.0 & 3.00 & $3.00\times1.2=3.60$ \\
\hline
合计             & 6.0  & ---  & 16.50 & 17.10 \\
\hline
\end{tabular}
...
\subsubsection{与模型的协同：提升 $R$ 的激励闭环}

本模型最核心的政策杠杆是废弃物转移率 $R$。积分体系通过行为干预，推动 $R$ 从基准值 $0.90$ 向上提升：

\begin{itemize}
    \item 消费者因积分奖励更倾向购买原本可能被遗弃的临期食品/非食品，零售商库存周转加快，实际废弃物减少，$R$ 提升。
    \item 敏感性分析显示，$R$ 每提升 $0.01$，年度净减排增量约 $1.4\ \mathrm{t\,CO_2e}$。若积分体系使 $R$ 升至 $0.95$，额外减排约 $7\ \mathrm{t\,CO_2e/年}$，相当于约 $780$ 棵成年乔木的年碳汇量。
\end{itemize}

因此，积分机制并非单纯的“返利”，而是直接嵌入节能减排的正反馈路径，在量化模型中具有可论证的减排增量。

\subsubsection{兑换方案与碳定价衔接}

表 \ref{tab:credit_redeem} 给出了四级兑换方案，其隐含碳价格保持在$100\text{--}125\ \text{元}/\mathrm{t\,CO_2}$，与当前国内碳试点市场价（约 $70\text{--}100$ 元）接近。

\begin{table}[htbp]
\centering
\caption{碳积分兑换方案}
\label{tab:credit_redeem}
\begin{tabular}{|c|l|l|}
\hline
所需积分 & 兑换内容 & 对应碳减排价值 \\
\hline
50 积分  & 兑换5元无门槛折扣券  & 约50\,kg\,CO$_2$e \\
\hline
100 积分 & 兑换12元折扣券 + “低碳达人”电子证书 & 约100\,kg\,CO$_2$e \\
\hline
200 积分 & 兑换25元折扣券 + 限量环保周边 & 约200\,kg\,CO$_2$e \\
\hline
500 积分 & 代捐赠碳抵消项目+个人年度碳足迹报告 & 约0.5吨CO$_2$e \\
\hline
\end{tabular}
\end{table}

根据分层估算（低频、中频、高频消费者分别占比约60\%、30\%、10\%），年度兑换总成本约占折扣店年流水的 $0.3\%\sim1\%$，企业可承受，并且通过滞销品折扣清仓带来的损失减少可以完全覆盖。

\subsubsection{小结}

本积分体系直接以模型中的避免排放量为积分锚点，兑换方案与现行碳交易市场接轨，在激励消费者参与减排的同时，通过提高实际废弃物转移率 $R$ 产生可量化的额外碳减排，兼具科学性和商业可行性。
...
\begin{enumerate}
    \item \textbf{方法论严谨，系统边界清晰。} 模型严格遵循《GHG Protocol》与 LCA 系统边界界定准则，将核算范围限定于初创公司直接运营活动（运输、仓储、消费者交通），上游供应链碳排放已包含在避免排放因子 $F_{avoid}$ 中，有效避免了双重计算。
    \item \textbf{多参数敏感性分析验证鲁棒性。} 任务二通过控制变量法对避免排放因子、废弃物转移率、运输参数及消费者交通排放因子逐一进行 $\pm20\%\sim\pm30\%$ 的敏感性测试，量化了各因素对净减排量 $E_{net}$ 的影响方向与幅度，验证了模型结果的稳健性。
    \item \textbf{模型通用性强，可迁移应用。} 本研究建立的是线性加和形式的净减排核算框架，不依赖特定商品品类或地区的微观参数。仅需替换 $M_{food}$、$M_{nonf}$ 与相应的排放因子 $F_{avoid,i}$ 等关键变量，即可迁移至其他品类折扣零售、社区团购等相似业态。
    \item \textbf{商业闭环设计完整。} 碳积分奖励机制打通了“环境贡献—用户激励—商业价值”的闭环，为初创公司的定价策略、营销推广与用户运营提供了科学的量化决策支持。
\end{enumerate}

\subsection{不足}

\begin{enumerate}
    \item \textbf{假设偏理想化。} 模型假设（"事后核算""理想情景""三折销售"）简化了现实中的价格波动、库存积压、损耗率等动态因素，可能导致 $E_{net}$ 估算偏离真实值。
    \item \textbf{缺乏实证数据校准。} 基准参数（如 $F_{avoid,food}=2.7$）虽引用自 Blonk 学术研究，但未使用该初创公司实际的商品碳足迹档案进行反向校核，参数有效性有待实际运营数据验证。
    \item \textbf{未考虑时间动态与季节性。} 食品类商品（如乳制品、烘焙食品）的消费与浪费行为在不同季节波动显著，当前年均化处理可能掩盖峰值期间的额外冷藏能耗。
    \item \textbf{未纳入消费者行为替代效应。} 消费者在购买折扣商品后，可能将节省的资金转向高碳消费（如航空旅行），形成间接碳排放的“回弹效应”，现阶段模型暂未覆盖。
\end{enumerate}

\subsection{模型展望}

\begin{enumerate}
    \item \textbf{补充国际贸易海运碳排放计算。} 根据题设数据（$10\%$ 欧洲进口、$30\%$ 东南亚进口），在系统边界中适度纳入海运阶段的碳排放，可更完整地反映供应链结构调整（如进口比例变化）对总净减排效益的影响。
    \item \textbf{推广到其他品类与场景。} 将当前食品+非食品的二分类模型扩展至多品类细分模型（如化妆品、清洁产品各设独立排放因子），并模拟不同品类组合下的最优折扣策略与排放减量效果。
    \item \textbf{引入蒙特卡洛模拟与动态优化。} 结合运营数据对关键参数进行概率分布拟合，采用蒙特卡洛模拟评估 $E_{net}$ 的概率分布区间，纳入库存动态、销售速率与消费者行为的时变特性，实现动态滚动预测与激励策略的自适应优化。
\end{enumerate}
...
\bibitem{wu2012} 吴燕, 王效科, 逯非. 北京市居民食物消费碳足迹[J]. 生态学报, 2012, 32(05): 1570-1577.
\bibitem{zhao2026} 赵汇, 林小英, 蒋伟, 等. 基于LCA-SD的城市生活垃圾碳排放分析及减排情景预测[J/OL]. 环境工程学报, 2026, 1-13 [2026-05-10]. https://link.cnki.net/urlid/11.5591.X.20260326.1810.004.
\bibitem{li2025} 李芳. 基于LCA的家居服产品碳足迹评价及其减排策略研究[D]. 北京化工大学, 2025. DOI:10.26939/d.cnki.gbhgu.2025.002127.
...
\subsubsection*{Python 源代码}
\lstinputlisting{sensitivity_analysis.py}
