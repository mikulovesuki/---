...
通过引入积分奖励机制，鼓励消费者进行消费，在年度净$CO_{2e}$减排模型基础上进一步减少废弃物和$CO_2$排放量
...
本项目聚焦于可持续发展背景下的新型零售商业模式的碳排放核算评估。该初创公司通过深度折扣销售临期与过剩商品，延长消费周期以刺激消费行为替代废弃处置，实现资源的高效利用，避免了资源的浪费和污染。题目的核心诉求是将这种“减少浪费”的商业行为所带来的环境效应进行估计和量化（即$CO_2$的减排量），并基于该模型设计出一套奖励机制进一步刺激消费。

为解决上诉问题，我们需要依托\textbf{生命周期评价$LCA$理论}与《GHG Protocol》 《Food Lossand Waste (FLW) Protocol》等国际公认标准，确立计算方程和系统边界。首先，建立碳排放核算方程；其次，代入案例数据计算出具体数值，并控制变量分析其各参量敏感度；最后，根据阶梯消费机制建立积分奖励机制。
\subsection{任务一的分析：净$CO_{2e}$减排模型的建立}

任务一要求本团队建立一套可靠的年度净$CO_{2e}$减排量模型，并且在此基础上对排放因子与假设选择的合理性进行验证分析。基于净碳减排公式$E_{net} = E_{avoid} - E_{add}$
...
$E_{net}$表示总净减排量
$E_{add}$表示增加的碳排放量
$E_{avoid}$表示通过销售所减少的碳排放量
$E_{add , trans}$表示运输过程产生的碳排放量
$E_{add , store}$表示存储产生的碳排放
$E_{add , move}$表示消费者交通产生的碳排放

\section{模型建立与求解}

\subsection{咕咕嘎嘎}%任务一模型

\subsubsection{食品类避免排放因子（取 $2.7\ kg\ CO_2e/kg$）的合理性}

\subsubsection{非食品类避免排放因子（取 $3.0\ kg\ CO_2e/kg$）的合理性}
