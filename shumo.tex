...
\subsubsection{消费者自提交通排放因子（取 $F_{move} = 238.6\ \mathrm{g\ CO}_2e/\mathrm{km}$）的合理性}

消费者前往实体门店自提的“最后一公里”出行方式具有多样性。本研究合理假设消费者的出行结构主要由“公共交通”与“零碳出行（如步行、自行车骑行）”构成，且两者比例各占 $50 \%$。基于广州市公共交通（以公交车辆综合耗能核算）每千米碳排放的平均水平（约 $477.2\ \mathrm{g\ CO}_2e/\mathrm{km}$），按 $50 \%$ 比例折算后，得出综合自提出行排放因子为 $238.6\ \mathrm{g\ CO}_2e/\mathrm{km}$。这一处理方式充分考量了短途自提场景下的真实绿色出行特征，使参数取值更为严密、合理。
...
\subsubsection{消费者年总消费次数（取 $N = 10000$ 次）合理性分析}

从题设提供的案例数据中，初创公司在2023年度的社区店日均订单量约为 27 单。基于此数据进行年度推算，得出消费者年总消费次数约为 $27 \times 365 = 9,855$ 次。为了简化计算并保持结果的可读性，我们将该数值四舍五入至 $10,000$ 次/年。这个取值既忠实反映了企业的实际运营情况，又为后续模型计算提供了一个清晰、易于理解的基准。
