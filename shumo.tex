...
各环节的具体数学表达与物理依据如下：
\begin{itemize}
    \item \textbf{运输过程产生的碳排放量 ($E_{add, trans}$)}：分为本地收集运输与进口海运两段。
    \begin{equation}
        E_{add, local} = 50 \times 20 \times F_{trans}
    \end{equation}
    \begin{equation}
        E_{add, sea} = \frac{M_{EU}}{1000} \cdot d_{EU} \cdot F_{sea} + \frac{M_{SEA}}{1000} \cdot d_{SEA} \cdot F_{sea}
    \end{equation}
    其中，$F_{trans}$ 为小型货车排放因子 ($\mathrm{kg\ CO_2e/km}$)，$F_{sea}$ 为海运排放因子 ($\mathrm{kg\ CO_2e/(t\cdot km)}$)，$M_{EU}, M_{SEA}$ 分别为欧洲与东南亚段货物质量，$d_{EU}, d_{SEA}$ 为对应航程。各参数取值在合理性小节中详细论证。
...
考虑到非食品类商品种类繁多，不同商品在生产、加工及销毁环节的碳足迹差异较大。为兼顾模型的科学性与计算的可操作性，本研究对各品类常见商品的生命周期碳排放数据进行了综合加权平均，最终将基准因子设定为 $3.0\ \mathrm{kg\ CO}_2e/\mathrm{kg}$。该均值能够较好地平抑个体差异，代表非食品类商品整体的碳排放基准水平。
...
\subsubsection{进口海运排放因子和海运航程设置的合理性}

海运排放因子 $F_{sea} = 0.015\ \mathrm{kg\ CO}_2\mathrm{e/(t\cdot km)}$ 取自国际海事组织（IMO）第四次温室气体研究报告中集装箱船平均排放水平。典型大型集装箱船（运力 $8000-12000\ \mathrm{TEU}$）在 $75\%$ 负载率下单位货物排放约为 $10-20\ \mathrm{g\ CO}_2\mathrm{e/(t\cdot km)}$，取中值 $15\ \mathrm{g}$ 即 $0.015$。该值与IMO目标（2030年降低至 $0.012$）方向一致且略保守，具有工程合理性。
海运航程依据国际海运常用航线实际里程估算：鹿特丹港至广州港（经马六甲海峡）约 $20{,}900\ \mathrm{km}$，新加坡港至广州港约 $2{,}800\ \mathrm{km}$，若两地到上海总路程相差$100\ \mathrm{km}$可忽略。货物分配比例 $10\%$ 来自欧洲、$30\%$ 来自东南亚的依据是：考虑到欧亚航线运输成本较高，预期欧洲货品以高附加值商品为主；东南亚作为近洋贸易带，运输成本低且品类匹配临期折扣的商品种类更丰富（日用品、纺织等），因此承载更大比例。$60\%$ 本土采购的假设与欧美进口比例形成互补，合计 $100\%$。

\subsubsection{消费者交通排放因子和消费者总消费次数设置的合理性}

消费者年总消费次数 $N$ 由年总销量 $60{,}000\,\mathrm{kg}$ 与平均每单购买量 $2.5\,\mathrm{kg}$（临期折扣零售常见客单）推算得 $N = 60{,}000 / 2.5 = 24{,}000$ 次/年。单次往返排放因子 $F_{move}$ 依据题目"往返 $1\,\mathrm{km}$、步行或公共交通"设定，假设 $50\%$ 公交 $50\%$ 步行，公交排放因子取 $0.037\,\mathrm{kg\ CO_2e/km}$（中国城市公交人均碳排放），步行排放为 $0$，则 $F_{move} = 1 \times (0.037\times 50\% + 0\times 50\%) \approx 0.02\,\mathrm{kg\ CO_2e/次}$。 
...
\begin{equation}
    E_{net} = \frac{\left[ \left( M_{food} F_{avoid, food} + M_{nonf} F_{avoid, nonf} \right) R_{waste} \right] - \left[ D_{total} F_{trans} + M_{food} F_{store} + N F_{move} \right]}{1000} \label{eq:final_net}
\end{equation}
